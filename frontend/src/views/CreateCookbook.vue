  ...
  <div class="max-w-3xl mx-auto px-4 pb-12">
    ...
    <div v-else>
      <div class="mb-8 border-b border-stone-200 pb-4">
        <h1 class="text-3xl sm:text-4xl font-serif font-bold text-stone-900">
          {{ isEditing ? "Editar Recetario" : "Nuevo Recetario" }}
        </h1>
        <p class="text-stone-500 mt-2">
          {{
            isEditing
              ? "Actualiza los detalles de tu recetario."
              : "Crea una colección para organizar tus recetas favoritas."
          }}
        </p>
      </div>

      <form
        @submit.prevent="submitCookbook"
        class="card p-10 space-y-8 border border-stone-200 shadow-none"
      >
        <div>
          <label
            class="block text-sm font-medium text-stone-700 mb-2 uppercase tracking-wide"
            >Título del Recetario</label
          >
          <input
            v-model="form.title"
            type="text"
            class="input-field text-lg font-serif placeholder-stone-300"
        ...
        <div>
          ...
          <textarea
            ...
            class="input-field min-h-[100px] leading-relaxed"
        ...
        <div>
          <h3
            class="block text-sm font-medium text-stone-700 mb-4 uppercase tracking-wide"
          >
            Seleccionar Recetas
          </h3>
          ...
          <div
            ...
            class="text-stone-500 italic text-sm p-4 bg-stone-50 rounded border border-stone-200 text-center"
          >
            No tienes recetas creadas aún.
            <router-link to="/create-recipe" class="text-olive-600 underline"
              >Crea una primero</router-link
            >.
          </div>

          <div
            v-else
            class="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[400px] overflow-y-auto p-1"
          >
            <div
            ...
            >
              ...
              <div class="ml-3">
                ...
                <span
                  v-if="
                    recipe.cookbook_id &&
                    recipe.cookbook_id !== parseInt(cookbookId)
                  "
