...
const loadRecipe = async () => {
  ...
  try {
    ...
    if (form.value.instructions_format === "numbered") {
      instructionSteps.value = recipe.instructions
...
const setInstructionFormat = (format) => {
  ...
  if (format === "numbered") {
    ...
    instructionSteps.value = form.value.instructions
      .split("\n")
...
const submitRecipe = async () => {
  try {
    ...
    alert(
      isEditing.value ? "Error actualizando receta" : "Error creando receta",
    );
  }
};
</script>
    ...
    <div class="max-w-4xl mx-auto">
      ...
      <div class="mb-8">
        ...
        <p class="text-stone-600 mt-2">
          Comparte tu creación culinaria con el mundo.
        </p>
      </div>

      <div v-if="loading" class="text-center py-12">
        <div
          class="animate-spin rounded-full h-12 w-12 border-b-2 border-olive-600 mx-auto"
        ></div>
      ...
      <form
        ...
        class="space-y-8 bg-white p-6 sm:p-8 rounded-xl shadow-sm border border-stone-200"
      >
        <!-- Basic Info & Image -->
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-1">
            <label
              class="block text-sm font-medium text-stone-700 uppercase tracking-wide mb-2"
              >Foto del Plato</label
            >
            <div
              class="relative group aspect-square bg-stone-100 rounded-lg overflow-hidden border-2 border-dashed border-stone-300 hover:border-olive-500 transition-colors flex items-center justify-center cursor-pointer"
            >
              <img
                v-if="form.image_url"
                :src="form.image_url"
              ...
              <div v-if="!form.image_url" class="text-center p-4">
                <svg
                  ...
                  stroke-width="1.5"
                  stroke="currentColor"
                  class="w-8 h-8 mx-auto text-stone-400"
                >
                  <path
                ...
                <span class="text-xs text-stone-500 mt-2 block">{{
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            <label
              class="block text-sm font-medium text-stone-700 uppercase tracking-wide"
              >Ingredientes</label
            >
          </div>
          ...
          <div class="space-y-3">
            <div
              ...
              :key="index"
              class="flex gap-2 items-start animate-fade-in-up"
            >
              <div class="grid grid-cols-2 sm:grid-cols-6 gap-2 flex-grow">
                <input
                ...
                <input
                  ...
                  class="input-field col-span-2 sm:col-span-4"
                />
              </div>
              <button
                type="button"
                @click="removeIngredient(index)"
                class="text-stone-400 hover:text-red-500 transition-colors bg-stone-50 p-2 rounded-md h-[42px] flex items-center justify-center border border-stone-200"
          ...
          <button
            type="button"
            @click="addIngredient"
            class="text-sm text-olive-600 font-medium hover:text-olive-700 mt-2 flex items-center gap-1"
          >
        ...
        <div>
          ...
          <div v-if="form.instructions_format === 'numbered'" class="space-y-3">
            ...
            >
              <span
                class="mt-3 text-stone-400 font-serif font-bold text-sm w-6 text-right select-none"
                >{{ index + 1 }}.</span
              >
              <div class="flex-grow">
                <textarea
              ...
              <button
                ...
                class="mt-3 text-stone-400 hover:text-red-500 transition-colors"
                title="Eliminar paso"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
          ...
          <div v-else>
            <textarea
              ...
              class="input-field min-h-[200px] leading-relaxed"
              required
              placeholder="Describe el proceso de preparación paso a paso..."
            ></textarea>
            <p class="text-xs text-stone-400 mt-1 italic">
              Escribe cada paso en una nueva línea.
            </p>
          </div>
        </div>
        ...
        <div class="bg-stone-50 p-6 rounded-md border border-stone-100">
          <label
            class="block text-sm font-medium text-stone-700 mb-2 uppercase tracking-wide"
            >Añadir a Recetario (Opcional)</label
          >
          <select
          ...
          >
            ...
            <option v-for="book in cookbooks" :key="book.id" :value="book.id">
              {{ book.title }}
            </option>
          </select>
        </div>
        ...
        <div>
          <label
            class="block text-sm font-medium text-stone-700 mb-2 uppercase tracking-wide"
            >Notas del Chef (Opcional)</label
          >
          <textarea
            v-model="form.notes"
            class="input-field min-h-[80px]"
            placeholder="Trucos, variaciones, historia..."
          ></textarea>
        </div>
        ...
        <div class="pt-4">
          <button
            type="submit"
            class="btn-primary w-full py-3 text-base shadow-sm hover:shadow-md transition-shadow"
          >
